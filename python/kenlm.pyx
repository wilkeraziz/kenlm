...
cdef class FullScoreReturn:
    ...
    def __cinit__(self, double log_prob, int ngram_length, bint oov):
...
cdef class Model:
    ...
    cpdef double BaseScore(self, State in_state, str word, State out_state):
        ...
        """
        cdef double total = self.model.BaseScore(&in_state._c_state, self.vocab.Index(as_str(word)), &out_state._c_state)
        return total
